...
class GQAFeatureDataset(Dataset):
    def __init__(self, args, name, dictionary, dataroot='data/gqa', adaptive=False):
        ...
        # load stat_word
        self.stat_words = json.load(open('/kaggle/working/lxmert%s_%s_stats_words.json' % (name, args.topk)))
        self.stat_skip_imgid = json.load(open('/kaggle/working/lxmert%s_%s_stats_skip_imgid.json' % (name, args.topk)))
        self.stat_features = {}

        # load attribute word
        self.attr_words = json.load(open('/kaggle/working/lxmert%s_attr_words_non_plural_words.json' % name))
        self.attr_skip_imgid = json.load(open('/kaggle/working/lxmert%s_attr_skip_imgid.json' % name))
        self.skip_imgid = []
        self.attr_features = {}
